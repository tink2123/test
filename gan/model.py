...
def build_resnet_block(inputres, dim, name="resnet", use_bias=False):
    out_res = fluid.layers.pad2d(inputres, [1, 1, 1, 1], mode="reflect")
    out_res = conv2d(out_res, dim, 3, 1, 0.02, name=name + "_c1", use_bias=use_bias)
    ...
    out_res = conv2d(
        out_res, dim, 3, 1, 0.02, name=name + "_c2", relu=False, use_bias=use_bias)
...
###    o_c6 = conv2d(o_c5, 3, 7, 1, 0.02, "SAME", name + "_c6", relu=False)
    o_c1 = conv2d(pad_input, 32, 7, 1, 0.02, name=name + "_c1")
    o_c2 = conv2d(o_c1, 64, 3, 2, 0.02, 1, name + "_c2")
    o_c3 = conv2d(o_c2, 128, 3, 2, 0.02, 1, name + "_c3")
    o_r1 = build_resnet_block(o_c3, 128, name + "_r1")
    o_r2 = build_resnet_block(o_r1, 128, name + "_r2")
    o_r3 = build_resnet_block(o_r2, 128, name + "_r3")
    o_r4 = build_resnet_block(o_r3, 128, name + "_r4")
    ...
    o_r8 = build_resnet_block(o_r7, 128, name + "_r8")
    o_r9 = build_resnet_block(o_r8, 128, name + "_r9")
    o_c4 = deconv2d(o_r9, 64, 3, 2, 0.02, [1,1], [0,1,0,1], name + "_c4")
    o_c5 = deconv2d(o_c4, 32, 3, 2, 0.02, [1,1], [0,1,0,1], name + "_c5")
    o_p2 = fluid.layers.pad2d(o_c5, [3, 3, 3, 3], mode="reflect")
    o_c6 = conv2d(o_p2, 3, 7, 1, 0.02, name = name + "_c6", norm=False, relu=False, use_bias=True)
...
def build_gen_discriminator(inputdisc, name="discriminator"):
    ...
    o_c2 = conv2d(o_c1, 128, 4, 2, 0.02, 1, name + "_c2", relufactor=0.2)
    o_c3 = conv2d(o_c2, 256, 4, 2, 0.02, 1, name + "_c3", relufactor=0.2)
    o_c4 = conv2d(o_c3, 512, 4, 1, 0.02, 1, name + "_c4", relufactor=0.2)
    o_c5 = conv2d(
        o_c4, 1, 4, 1, 0.02, 1, name + "_c5", norm=False, relu=False, use_bias=True)
    return o_c5
