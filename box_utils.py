    ...
    for boxes, labels, scores in zip(pred_boxes, pred_labels, pred_scores):
        ...
        for c in np.unique(labels):
            ...
            for i in range(len(keep)):
                if dets[i][-1]>valid_thresh:
                    nms_boxes = []
                    nms_scores = []
                    nms_labels = []
                    nms_scores.append(nms_dets[i][-1])
                    nms_labels.append(c)
                    nms_boxes.append(nms_dets[i][:4])
                    result_boxes = np.append(result_boxes,nms_boxes, axis=0)
                    result_scores = np.append(result_scores, nms_scores)
                    result_labels = np.append(result_labels, nms_labels)
                    i += 1
                else:
                    break
